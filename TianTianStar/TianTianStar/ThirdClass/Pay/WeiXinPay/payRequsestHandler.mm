...
#pragma mark - FX 微信支付
- ( NSMutableDictionary *)sendPay:(NSString *)goodName price:(NSInteger )price orderno:(NSString *)orderno notify_url:(NSString *)notify_url
{
    
    //订单标题，展示给用户
    NSString *order_name    = goodName;
    //订单金额,单位（分）
    NSString *order_price   = [NSString stringWithFormat:@"%zd",price]; //1分钱测试
...
//    NSString *orderno   = [NSString stringWithFormat:@"%ld",time(0)];
    ...
    [packageParams setObject: appid             forKey:@"appid"];       //开放平台appid
    [packageParams setObject: mchid             forKey:@"mch_id"];      //商户号
    [packageParams setObject: @"iOS-baihong"        forKey:@"device_info"]; //支付设备号或门店号
    [packageParams setObject: noncestr          forKey:@"nonce_str"];   //随机串
    [packageParams setObject: @"APP"            forKey:@"trade_type"];  //支付类型，固定为APP
    [packageParams setObject: order_name        forKey:@"body"];        //订单描述，展示给用户
